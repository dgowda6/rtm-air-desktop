...
Ext.extend(Ext.data.SqlDB, Ext.util.Observable, {
	...
	createTable : function(o){
		...
		var scope = o.scope;
		var buf = [];
		for(var i = 0, len = fs.getCount(); i < len; i++){
			var f = fs.get(i), s = f.name;
			switch(f.type){
	            case "int":
	            case "bool":
	            case "boolean":
...
Ext.data.SqlDB.Proxy = function(conn, table, keyName, storeConfig){
    Ext.data.SqlDB.Proxy.superclass.constructor.call(this);
    this.conn = conn;
	this.conn.createTable({
		name: table,
		key: keyName,
		fields: storeConfig.recordType.prototype.fields
	});

    this.table = this.conn.getTable(table, keyName);
	storeConfig.proxy = this;
    this.store = new Ext.data.ArrayStore(storeConfig);
	this.store.reader = new Ext.data.JsonReader({
            idProperty: keyName
        }, storeConfig.recordType);
	this.store.data.getKey = function(o){
//		air.trace('getKey(mod)', keyName, o.get(keyName));
		return o.get(keyName);
	}
...
Ext.extend(Ext.data.SqlDB.Proxy, Ext.data.DataProxy, {
    ...
    onLoad : function(trans, rs, e, stmt){
		...
		//air.trace('onLoad', rs? rs.length: 0);
		//for(var id = 0; id<(rs? rs.length: 0); id++){
		//	air.trace('rs', id, rs[id].task_id, rs[id].seconds);
		//}
